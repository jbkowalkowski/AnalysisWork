...
#----------------------------------------------------------
# slices wires and truth arrays into sequencial batches
	...
	random.seed(10)
		...
		for k in range(0,len(datafiles[j])):
			...
			for l in range(batchsize,len(data)+1,batchsize):
					...
					if bidirectional: wireprobabilities =(model.predict_proba([d,d],batch_size=batchsize,verbose=0))
					else: wireprobabilities =(model.predict_proba(d,batch_size=batchsize,verbose=0))
					model.reset_states()
