...
if len(sys.argv) != 2:
		...
		padding = numattributes//2
                assert padding*2+1 == numattributes
                padarray = np.zeros((padding,wirelength),dtype=np.short)
...
def parse_attributes(data):
        d = list()
        for i in range(0,len(data)-numattributes+1):
                d.append(np.swapaxes(data[i:i+numattributes],0,1))
        return np.asarray(d)
	...
	random.seed(10)
		...
		for k in range(0,len(datafiles[j])):
			...
			for l in range(batchsize,len(truth)+1,batchsize):
					...
					print len(wireprobabilities),len(wireprobabilities[0]),len(wireprobabilities[0][0]), len(t)	
